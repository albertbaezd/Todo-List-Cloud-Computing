...
# gcloud compute ssh $INSTANCE_NAME --zone $ZONE --command "echo `pip`"

# # Install Flask using pip on VM
# gcloud compute ssh $INSTANCE_NAME --zone $ZONE --command "sudo pip install Flask"
# gcloud compute ssh test-instance-1 --zone us-central1-a --command "sudo pip install Flask"

# Install Flask using pip on VM
# gcloud compute ssh $INSTANCE_NAME --zone $ZONE --command "echo `which pip` `whoami`"
...
# gcloud compute ssh $INSTANCE_NAME --zone $ZONE --command "python3 -m venv $VIRTUAL_ENV_DIR"
